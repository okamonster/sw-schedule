...
- **カレンダー表示機能**
  - 推しの出演日ハイライト
  - 月別・週別表示
  - **推しのアーティストの全イベントをGoogleカレンダーに一括同期** ✅ 実装完了
- **ユーザー認証・推し登録機能**
  - ユーザー登録・ログイン
  - 推し登録・管理
  - プッシュ通知設定
...
### 2025-07-01 イベント関連機能 実装完了
- イベント詳細表示、編集、地図表示（住所からのジオコーディング含む）など、イベント管理に関する主要機能を実装完了。

### 2025-07-15 退会機能の実装
- ユーザー退会機能を実装
- DeletedUserテーブルを作成し、削除されたユーザーデータを安全に保存
- トランザクション処理による安全なデータ移行
- 退会確認フォーム（メールアドレス、パスワード、確認テキスト）
- 関連データ（推し登録、プロフィール）の適切な削除
